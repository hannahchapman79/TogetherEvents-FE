...
import { LuPalette, LuBookOpenText, LuPartyPopper, LuUsersRound } from "react-icons/lu";
...
export default function CategoryCircles() {
    const categories = [
        ...
        { name: "Book Club", slug: "bookclub", icon: <LuBookOpenText size={28   } /> },
        { name: "Networking", slug: "networking", icon: <LuUsersRound size={28  } /> },
        { name: "Entertainment", slug: "entertainment", icon: <LuPartyPopper size={28   } /> },
        { name: "Health", slug: "health", icon: <FaRegHeart size={28    } /> },
        { name: "Outdoors", slug: "outdoors", icon: <TiTree size={28    } /> },
        { name: "Education", slug: "education", icon: <IoSchoolOutline size={28 } /> }
    ];
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 place-items-center">
                {categories.map((category) => (
                    ...
                    >
                        ...
                        <span className="text-gray-800 text-sm text-center sm:text-xs">{category.name}</span>
                    </Link>
                ))}
            </div>
        </div>
